### Inspired from https://github.com/kimjeyoung/SNGP-BERT-Pytorch/blob/main/data_loader.py
import os
import torch
import pandas as pd
import numpy as np
...
class PapyrusLoader(Dataset):
    def __init__(self,
        ...
        self.num_classes = len(np.unique(self.y_data)) + 1 if train_or_test in ['train', 'val'] else len(
            np.unique(self.y_data))
...
class BioactivityDataset(Dataset):
    def __init__(self, csv_file):
        self.bioactivities = pd.read_csv(csv_file)

    def __len__(self):
        return len(self.bioactivities)

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()

        mol_id = self.bioactivities.iloc[idx, 0]
        activity = self.bioactivities.iloc[idx, 1:].values.astype('float32')

        sample = {'mol_id': mol_id, 'activity': activity}

        return sample

# Example usage
bioactivity_dataset = BioactivityDataset('bioactivity.csv')
bioactivity_dataloader = DataLoader(bioactivity_dataset, batch_size=32, shuffle=True)
