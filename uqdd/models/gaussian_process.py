...
from uqdd.models.baseline import MTBaselineDNN, run_epoch, predict
...
def build_gp_model(config=wandb.config, kernel="tanimoto"):
    ...
    m = gpflow.models.GPR(
        data=(X_train, y_train),
        mean_function=Constant(-1.7),
        kernel=cst * k,
        noise_variance=0.056,
    )
...
#     save_model, calc_regr_metrics, set_seed, MultiTaskLoss
#
# today = date.today()
# today = today.strftime("%Y%m%d")
#
