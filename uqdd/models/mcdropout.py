...
def predict(
    ...
    targets_all = []
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(tqdm(test_loader, desc='MC prediction', total=len(test_loader))):
            inputs = inputs.to(device)
            outputs = torch.stack([model(inputs) for _ in range(num_samples)], dim=2)  # Multiple forward passes
...
def run_mcdropout(
    ...
    with wandb.init(
        ...
        preds, targets = predict(best_model.to(device), test_loader, num_samples=config.num_samples, return_targets=True)
        ...
        for task_idx in range(len(tasks)):
            task_y_pred, task_y_std, task_y_true = process_preds(preds, targets, task_idx=task_idx)
